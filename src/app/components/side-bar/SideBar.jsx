  ...
  const renderNavLink = (item) => {
    const linkContent = (
      <div className='flex items-center gap-3 w-full text-right'>
        <span className='order-2 text-2xl'>{item.icon()}</span>
        ...
        className={`flex items-center w-full p-1 text-gray-900 dark:text-gray-200 transition duration-100 rounded-lg group 
          ${
            isExpanded
              ...
              : 'text-base font-medium justify-center'
          }
          ${
            isActive(item.link)
  ...
  return (
    <div className='relative z-50'>
      ...
      {(isExpanded || !isMobile) && (
        ...
        >
          <div
            className={`h-full px-2 py-3 overflow-y-auto bg-white dark:bg-gray-900 border-x border-gray-200 dark:border-gray-700 shadow-sm fixed right-0 ${styles.sidebar} ${styles.sidebarOpen}`}
          >
            ...
            {state?.userData?.userId && (
              <ul className='space-y-3'>
                {links.map((item) => {
                  ...
                  if (item.link === 'administration' && !isAdminOrTeamLead) {
                    // If the link is "administration" and the user is not an admin or teamlead, skip rendering.
                    return null
                  }

                  // Render the navigation link for valid cases.
